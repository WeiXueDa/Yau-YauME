...
def safe_eval_func(func_str, var_names):
    ...
    try:
        if len(var_names) == 2:
            return eval(f"lambda {var_names[0]},{var_names[1]}: {func_str}")
        else:
...
def simulate_processes_qiem(f_func, h_func, sigma_theta, sigma_S, T, Dt, Dtau, seed):
    np.random.seed(seed)
    Nt = int(Dtau / Dt)
    Ntau = int(T / Dtau)
    NtNtau = Ntau * Nt
    ...
    for t in range(1, NtNtau):
        ...
        theta_true[t] = theta_true[t-1] + drift_theta * Dt + sigma_theta * np.sqrt(Dt) * np.random.randn()
...
def run_qiem_filter_dst_1d(s, n_total, n_internal, L_1d, K_1d, y_tau, f_func, h_func, Ds, Dt, Dtau):
    s_internal = s[1:-1]
    initial_theta = 1.0
...
with st.sidebar:
    ...
    if method == "Hermite Spectral Method":
        ...
        Dtau = st.number_input("Dtau (Observation Interval)", value=0.005, min_value=0.001, max_value=0.01, step=0.001,format="%.3f")
        Ds = st.number_input("Ds (Spatial Step)", value=0.5, min_value=0.01, max_value=1.0, step=0.01,format="%.3f")
        R = st.number_input("R (Spatial Range)", value=15.0, min_value=10.0, max_value=30.0, step=1.0)
        ...
        mu1 = 0.3
        omega = 1.0
        sigma_theta = float(st.text_input("σ_theta (State Noise)", value="0.70"))
        sigma_S = float(st.text_input("σ_S (Observation Noise)", value="0.50"))
        
        # 稳定性检查
        p_max = abs(-(mu0 + mu1))
        q_max = -0.5 * ((R - 0.5*sigma_S**2) ** 2)
        ...
        params = {
            ...
            "Ds": Ds, "R": R, "sigma_theta": sigma_theta,
            "sigma_S": sigma_S, "Nt": Nt, "Ntau": Ntau,
            "NtNtau": NtNtau, "mu0": mu0, "mu1": mu1, "omega": omega
        }
...
if run_btn:
    with st.spinner("Running simulation and filtering..."):
        if method == "Hermite Spectral Method":
            ...
            # 定义模型函数（与参考代码一致）
            def f_func(theta, t=0):
                return params["mu0"] + params["mu1"] * np.sin(params["omega"] * t)
            
            def h_func(theta):
                return theta - 0.5 * params["sigma_S"] ** 2
            ...
            theta_true, y_full, y_tau = simulate_processes_qiem(
                f_func, h_func, params["sigma_theta"], params["sigma_S"],
